...
namespace TagerCom.Areas.Customer.Controllers
{
    ...
    public class OrdersController : ControllerBase
    {
        ...
        public async Task<IActionResult> TrackOrder(Guid id)
        {
            ...
            var dto = new OrderTrackDTO
            {
                ...
                CurrentStatus = (order.StatusHistory
                    .OrderByDescending(h => h.ChangedAt)
                    .FirstOrDefault()?.Status ?? order.OrderStatus).ToString(),


                TotalAmount = order.TotalAmount,
                CreatedAt = order.CreatedAt,
                VendorName = order.Store?.StoreName ?? "Unknown Vendor",
        ...
        public async Task<IActionResult> CancelOrderItem(Guid orderId, Guid itemId)
        {
            ...
            // Update order status ------------------------------------------
            //order.OrderStatus = order.OrderItems.Any() ? "PartiallyCancelled" : "Cancelled";
            order.OrderStatus = order.OrderItems.Any() ? OrderStatus.Completed : OrderStatus.Cancelled;
            ...
            return Ok(new { message = "Item cancelled successfully", orderStatus = order.OrderStatus });
        }
